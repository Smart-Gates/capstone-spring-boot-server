...
public class MeetingController {
	...
	public MeetingController(MeetingRepository meetingRepository, MeetingResourceAssembler assembler) {
	...
	public CollectionModel<EntityModel<Meeting>> all() {
		...
		List<EntityModel<Meeting>> meetings = meetingRepository.findAll().stream().map(assembler::toModel)
				.collect(Collectors.toList());

		return new CollectionModel<>(meetings, linkTo(methodOn(MeetingController.class).all()).withSelfRel());
	}

	@GetMapping("/meetings/{id}")
	public EntityModel<Meeting> one(@PathVariable Long id) {
		return assembler.toModel(meetingRepository.findById(id).orElseThrow(() -> new MeetingNotFoundException(id)));
	}
	...
	public ResponseEntity<EntityModel<Meeting>> newMeeting(@RequestBody Meeting meeting) {
		...
		return ResponseEntity.created(linkTo(methodOn(MeetingController.class).one(newMeeting.getId())).toUri())
	...
	public ResponseEntity<RepresentationModel> cancel(@PathVariable Long id) {
		...
		return ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED).body(new VndErrors.VndError("Method not allowed",
				"You can't cancel an meeting that is in the " + meeting.getStatus() + " status"));
	}
	...
	public ResponseEntity<RepresentationModel> complete(@PathVariable Long id) {
		...
		return ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED).body(new VndErrors.VndError("Method not allowed",
				"You can't complete an meeting that is in the " + meeting.getStatus() + " status"));
	}
}
